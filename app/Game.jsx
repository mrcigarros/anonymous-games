...
// ── Daily Schedule (UTC) ─────────────────────────────────────────
const SCHEDULE = [
  { id: 1, name: "The Bigger Daily Major Bingo", price: 10, utcHour: 0, jackpotBase: 42100, emoji: "👑", tier: "premium" },
  { id: 2, name: "The Five Dollar Bingo", price: 5, utcHour: 6, jackpotBase: 18900, emoji: "💵", tier: "mid" },
  { id: 3, name: "The Mini Daily Major Bingo", price: 1, utcHour: 12, jackpotBase: 4250, emoji: "🎰", tier: "entry" },
  { id: 4, name: "The One Dollar Bingo", price: 1, utcHour: 18, jackpotBase: 3100, emoji: "🎲", tier: "entry" },
  { id: 5, name: "The Big Daily Dollar Bingo", price: 5, utcHour: 22, jackpotBase: 15600, emoji: "💰", tier: "mid" },
];

function getGameStatus(utcHour) {
  const now = new Date();
  const utcNow = now.getUTCHours() * 60 + now.getUTCMinutes();
  const gameTime = utcHour * 60;
  let diff = gameTime - utcNow;
  if (diff < -720) diff += 1440; // wrap around midnight
  const gameDuration = 45;
  if (diff <= 0 && diff > -gameDuration) {
    const elapsed = Math.abs(diff);
    return { status: "live", progress: Math.min(Math.floor((elapsed / gameDuration) * 75), 75), startedAgo: elapsed + " min ago", players: 50 + Math.floor(Math.random() * 150), cards: 400 + Math.floor(Math.random() * 2000) };
  } else if (diff > 0 && diff <= 60) {
    return { status: "registering", timeUntil: diff >= 60 ? Math.floor(diff/60) + "h " + (diff%60) + "m" : diff + "m", players: 10 + Math.floor(Math.random() * 60), cards: 50 + Math.floor(Math.random() * 400) };
  } else if (diff <= -gameDuration) {
    return { status: "completed", players: 80 + Math.floor(Math.random() * 200), cards: 600 + Math.floor(Math.random() * 3000), prize: Math.floor(Math.random() * 3000) + 500, winner: Math.random().toString(36).slice(2,5) + "..." + Math.random().toString(36).slice(2,5) };
  } else {
    if (diff < 0) diff += 1440;
    return { status: "upcoming", timeUntil: Math.floor(diff/60) + "h " + (diff%60) + "m", players: 0, cards: 0 };
  }
}
...
function buildGames() {
  return SCHEDULE.map(s => ({ ...s, currency: "USDC", ...getGameStatus(s.utcHour), jackpot: s.jackpotBase + Math.floor(Math.random() * 1000), localTime: utcToLocal(s.utcHour), utcTime: String(s.utcHour).padStart(2,"0") + ":00 UTC" }));
}
...
export default function AnonymousGaming() {
  ...
  const cloakProcessing = false, cloakProgress = "", proofProgress = 0;
  ...
  useEffect(() => { setLobbyGames(buildGames()); const iv = setInterval(() => setLobbyGames(buildGames()), 30000); return () => clearInterval(iv); }, []);
  const [selGameData, setSelGameData] = useState({ name: "Daily Dollar", price: 1, jackpot: 4250 });
  ...
  const buyCards = async () => {
    ...
    // Cloak integration on standby — will be activated post-hackathon
  ...
  const prizePool = totalCards * selGameData.price * 0.8;
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "0 16px", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ background: `linear-gradient(135deg, ${C.gold}12, ${C.accent}08)`, border: `1px solid ${C.gold}25`, borderRadius: 16, padding: "16px 20px", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "var(--fH)", color: C.gold }}>${selGameData.jackpot.toLocaleString()}</div>
        ...
        {/* Daily Schedule */}
        <div style={{ fontSize: 10, fontWeight: 700, color: C.muted, letterSpacing: 2, marginBottom: 8 }}>DAILY SCHEDULE ({new Date().toLocaleDateString([], {weekday:"short",month:"short",day:"numeric"})})</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
          {lobbyGames.map(game => {
            const isLive = game.status === "live";
            const isDone = game.status === "completed";
            const isOpen = game.status === "registering";
            const tierColor = game.price >= 10 ? C.gold : game.price >= 5 ? C.accent : C.cyan;
            return (
              <button key={game.id} onClick={() => {
                if (isLive) { setSelGameData(game); setSelGame(game); setView("game"); }
                else if (isOpen) { setSelGameData(game); startPurchase(); }
              }} style={{
                width: "100%", padding: 0, border: `1px solid ${isLive ? tierColor + "40" : C.border}`,
                borderRadius: 14, background: C.card, cursor: isLive || isOpen ? "pointer" : "default", textAlign: "left",
                overflow: "hidden", transition: "all .2s", opacity: isDone ? 0.6 : 1,
                boxShadow: isLive ? `0 0 20px ${tierColor}10` : "none",
              }}>
                {/* Top */}
                <div style={{ padding: "14px 16px 10px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      ...
                      <div style={{ fontSize: 15, fontWeight: 800, color: C.text, fontFamily: "var(--fH)" }}>{game.name}</div>
                    </div>
                    <div style={{ fontSize: 11, color: C.muted }}>
                      <span style={{ color: tierColor, fontWeight: 700, fontSize: 13 }}>${game.price}</span> USDC · <span style={{ color: C.cyan }}>{game.localTime}</span> <span style={{ fontSize: 9 }}>({game.utcTime})</span>
                    </div>
                  </div>
                ...
                {isLive && (
                  <div style={{ padding: "0 16px 8px" }}>
                    <div style={{ height: 4, borderRadius: 2, background: `${tierColor}20`, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: (game.progress / 75 * 100) + "%", borderRadius: 2, background: `linear-gradient(90deg, ${tierColor}, ${C.cyan})` }} />
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between", marginTop: 3, fontSize: 9, color: C.muted }}>
                      <span>Draw {game.progress}/75</span><span>Started {game.startedAgo}</span>
                    </div>
                  </div>
                )}
                {/* Stats */}
                <div style={{ display: "flex", borderTop: `1px solid ${C.border}40`, padding: "8px 16px" }}>
                  <div style={{ flex: 1 }}><div style={{ fontSize: 9, color: C.muted, letterSpacing: 1 }}>PLAYERS</div><div style={{ fontSize: 14, fontWeight: 700, fontFamily: "var(--fH)" }}>{game.players || "—"}</div></div>
                  <div style={{ flex: 1 }}><div style={{ fontSize: 9, color: C.muted, letterSpacing: 1 }}>CARDS</div><div style={{ fontSize: 14, fontWeight: 700, fontFamily: "var(--fH)" }}>{game.cards ? game.cards.toLocaleString() : "—"}</div></div>
                  <div style={{ flex: 1, textAlign: "right" }}><div style={{ fontSize: 9, color: C.gold, letterSpacing: 1 }}>JACKPOT</div><div style={{ fontSize: 14, fontWeight: 700, color: C.gold, fontFamily: "var(--fH)" }}>${game.jackpot.toLocaleString()}</div></div>
                </div>
                {/* Status hint */}
                {isOpen && <div style={{ padding: "6px 16px 10px", fontSize: 11, color: C.blue }}>Registration open · Starts in {game.timeUntil}</div>}
                {game.status === "upcoming" && <div style={{ padding: "6px 16px 10px", fontSize: 11, color: C.muted }}>Opens in {game.timeUntil}</div>}
                {isDone && <div style={{ padding: "6px 16px 10px", fontSize: 10, color: C.muted }}>Winner: <span style={{ color: C.green, fontFamily: "var(--fM)" }}>{game.winner}</span> · Prize: <span style={{ color: C.gold }}>${game.prize?.toLocaleString()}</span></div>}
              </button>
            );
          })}
        </div>
  ...
  if (view === "purchase") return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "var(--fB)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "0 16px", width: "100%", flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
        <div style={{ fontSize: 22, fontWeight: 800, fontFamily: "var(--fH)", marginBottom: 4 }}>{selGameData.name}</div>
        <div style={{ fontSize: 12, color: C.muted, marginBottom: 8 }}>${selGameData.price} USDC per card · Max 50 cards</div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: C.card, borderRadius: 12, marginBottom: 14, border: `1px solid ${C.border}` }}>
          ...
          <span style={{ fontSize: 24, fontWeight: 800, color: C.gold, fontFamily: "var(--fH)" }}>${(playerCardCount * selGameData.price).toFixed(2)} <span style={{ fontSize: 12, color: C.muted }}>USDC</span></span>
        </div>

        <div style={{ display: "flex", gap: 8, marginBottom: 18, fontSize: 10, color: C.muted }}>
          {[["Prize Pool", (playerCardCount * selGameData.price * .8).toFixed(2), C.green], ["Jackpot", (playerCardCount * selGameData.price * .15).toFixed(2), C.gold], ["House", (playerCardCount * selGameData.price * .05).toFixed(2), C.muted]].map(([l, v, c]) => (
        ...
        <div style={{ textAlign: "center", marginTop: 10, fontSize: 10, color: C.muted }}>🎱 Cards will be generated on-chain via Switchboard VRF</div>
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "var(--fB)" }}>
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "0 16px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: `1px solid ${C.border}50` }}>
          <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "var(--fH)" }}>{selGameData.name}</div>
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "var(--fB)" }}>
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "0 16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 0", borderBottom: `1px solid ${C.border}50` }}>
          <div style={{ fontSize: 14, fontWeight: 700, fontFamily: "var(--fH)" }}>{selGameData.name}</div>
          <Badge status="live" />
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", background: `${C.gold}08`, border: `1px solid ${C.gold}20`, borderRadius: 10, marginBottom: 8, fontSize: 11 }}>
          <span style={{ color: C.gold }}>🎱 Jackpot ${selGameData.jackpot.toLocaleString()}</span>
